...
- **Перенос перед `)`/`}`** — скобка уезжает на свою строку с отступом строки, где живёт **парная открывающая** (обратный скан баланса `(`/`{` vs `)`/`}` от позиции курсора; fallback −sw при непарной). Выравнивание по открывающей, а не «минус один отступ», переживает ручные перепроме́ты и многострочные вызовы
