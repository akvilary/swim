...
**Специальные команды:** `cd <dir>` — меняет рабочую директорию окна (каждая команда живёт в своём sh, поэтому cwd трекает само окно; поддерживаются `~` и относительные пути), `clear`/`cls` и `Ctrl+L` — очистить вывод.
