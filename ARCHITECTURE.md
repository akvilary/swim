...
- `Ctrl+X` — закрыть текущую вкладку (семантика `:q`: отказ при несохранённых правках, выход при последней), `Ctrl+Z` — закрыть все остальные (модифицированные сохраняются с сообщением)
