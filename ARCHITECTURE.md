...
- `Ctrl+X` — закрыть текущую вкладку (семантика `:q`: отказ при несохранённых правках, выход при последней), `Ctrl+Z` — закрыть все остальные, кроме несохранённых (они остаются с сообщением)
...
**Управление:**
- `j/k` — вверх/вниз
- `Enter/l/Ctrl+Right (Ctrl+L)` — развернуть директорию и перейти в её первый дочерний элемент; для файла — открыть
...
- За экраном ждёт чистая вкладка `[No Name]` — первый открытый файл заменяет её; открытие файла (`ensureEditorVisible()`) делает редактор видимым, и проводник обрезается до сайдбара
- Пока редактор закрыт: `:`, `Ctrl+X`, `Ctrl+Z` не действуют; `Ctrl+G` показывает git-панель снизу без открытия редактора; `Ctrl+E`/`Escape` не сворачивают проводник — нечего показать вместо него; `Escape` сворачивает другие панели, если остаётся хоть одно видимое окно
...
- `Ctrl+F` — поиск
- `Ctrl+C` — выход
- `Ctrl+S` — текущая панель на половину экрана по своей оси / обратно (проводник и результаты поиска — половина ширины, git-панель и командное окно — половина высоты, превью — половина ширины); не действует в редакторе
...
- Git-панель / командное окно: до 25 строк или 1/2 высоты, снизу (`Ctrl+S` — ровно половина высоты)
