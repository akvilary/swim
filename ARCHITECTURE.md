...
Панель терминала (Ctrl+T или `:terminal`/`:term`/`:sh`): ввод shell-команд и просмотр вывода, не покидая редактор. История вывода — на тёмно-сером фоне `Theme.terminalBg`, отличном от фона остальных окон; строка ввода — на стандартном фоне заголовка (`bgHighlight`).

**Ввод:** живое приглашение `dir>` — последняя строка прокручиваемого буфера, фиксированной строки ввода нет. Редактирование строки (`left/right/home/end/backspace/delete`), `Ctrl+Left`/`Ctrl+Right` — история команд, `Tab` — автодополнение пути (одно совпадение подставляется целиком, для директорий с `/`; несколько — общий префикс), `Enter` — запуск.
