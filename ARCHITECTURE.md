...
while running:
    poll LSP tokens
    tick spinners (git-задачи, поиск)
    if input available:
        parse key
        handle global key
...
**Пространства (`Spaces`):** два space'а — `editor` (проводник + таб-бар + редактор + git-панель + командное окно + статус-бар) и `search` (результаты поиска + превью + статус-бар, вызов по Ctrl+F). Окна поиска не входят в стек editor-space: выход из поиска (Escape/Ctrl+X/Ctrl+F или открытие файла) возвращает фокус на прежнюю вершину стека.
...
**Командная строка из любого окна:** `:` не крадёт фокус — панель остаётся вершиной стека, ввод отображается в статус-баре, `:q` закрывает сфокусированное окно. Блокируется только в insert-режиме редактора и при вводе поискового запроса (`inputMode` — там `:` печатается буквально). Команда `:e <file>` при скрытом редакторе раскрывает его (`ensureEditorVisible`) и поднимает на вершину стека; открытие файла из проводника/поиска/перехода `gd` — так же.
...
| `:` | Командная строка из любого окна (кроме insert и ввода поискового запроса) |
